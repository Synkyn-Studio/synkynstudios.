                    ...
                    <li>
                        <a class="group relative flex items-center justify-between rounded-[14px] border border-transparent dark:border-white/5 bg-[#1a1a1a] p-4 transition-all duration-300 hover:border-white/10 hover:bg-[#252525]" href="./printalbum.html">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col text-left">
                                    <p class="text-[15px] font-medium text-white leading-tight">Print Album</p>
...
/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   Clicking "Home" while already on Home used to trigger a full document
   navigation: the browser tore down and rebuilt the page, replayed the hero
   reveal and re-booted the Vimeo player, all to arrive exactly where the
   visitor already was. Same for Library, Print Album, Contact and Terms.

   This intercepts those clicks. If the link resolves to the page already
   open, the navigation is cancelled and we scroll to the top instead —
   which is almost always what someone means when they click the logo or
   the current section.

   Paths are normalised first, because "/" , "/index.html" and "./" all
   address the same document and a naive string compare would miss it.

   Links carrying a hash for a DIFFERENT page, external links, new-tab
   clicks and modified clicks (Ctrl / Cmd / middle-button) are all left
   alone.
   ========================================================================== */
(function () {
    'use strict';
    ...
    function isSamePage(link) {
        ...
        return target !== null && target === normalise(window.location.href);
    }
    ...
    document.addEventListener('click', function (e) {
        // Leave modified clicks to the browser — Ctrl/Cmd/middle open a tab.
        ...
        // A hash means "go to this section" — let the existing hash handler run.
        var hash = '';
        ...
        // Close any open desktop dropdown so the header settles.
        ...
        // Close the mobile panel if it happens to be open.
    ...
    /* Mark the link for the page we are on, so it can be styled as current. */
    function markCurrent() {
        var here = normalise(window.location.href);
        document.querySelectorAll('#desktop-navbar a[href], .header-two a[href]')
            .forEach(function (a) {
